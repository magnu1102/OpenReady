...
- **Phase 18 — CI gate and GitHub Action.** Composite action wrapping the CLI gating, score badges from the JSON export, example workflows. Post-push: optional Marketplace listing and a self-badge refresh workflow, both after tagging `v0.3.0`. ✅
- **Phase 19 — Distribution hardening.** npm packaging of the CLI, updater groundwork, signing/notarization checklist, Rust checks in CI, e2e smoke test. ← _current_
