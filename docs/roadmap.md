...
- **Phase 19 — Aurora: visual and text revamp.** Glass design language (translucent surfaces, violet-blue accent, gradient glow), full framer-motion choreography, confident-and-precise copy rewrite with a central copy module and voice guide, toast system, floating sidebar rail, new brand mark and icons, README restructure with fresh screenshots. Ships as v0.4.0. ← _current_
- **Phase 20 — Distribution hardening.** npm packaging of the CLI, updater groundwork, signing/notarization checklist, Rust checks in CI, e2e smoke test.
- **Phase 21 — GitHub client efficiency and org support.** ETag conditional requests, request-budget indicator, optional GraphQL path, organization analysis, changed-repo-only refresh.
- **Phase 22 — History and trends.** Bounded snapshot history, score deltas since last analysis, trend views, progress-report export.
- **Phase 23 — Local AI providers.** Ollama via the OpenAI-compatible adapter, provider presets, local-model badge, graceful degradation for small models.
