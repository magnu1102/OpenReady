...
OpenReady desktop
  ...
  │   ├── store/         — Zustand slices (theme, navigation, repository fetch state)
...
The `github-client` module now fetches public repository metadata for a GitHub user. The other `src/modules/*` folders define the public surface of future packages. When the project grows beyond a single app, they can be lifted into a `packages/*` workspace without rewriting their consumers.
